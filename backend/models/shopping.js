...
function getCart(userID,cb){
	cartModel.findRule({userID:obj.userID},function(err,docs){
		cb(docs[0].goodsList);
	})
}
function addtoCart(obj,cb){
	...
	cartModel.find({userID:obj.userID},function(err,docs){
		docs[0].goodsList.push(newobject)
		docs[0].save();
	})
}
function deleteItemFromCart(userID,goodsID,cb){
	cartModel.find({userID:userID},function(err,docs){
